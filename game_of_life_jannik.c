...
void evolve_mpi_parallel(double* currentfield, double* newfield, int w, int h){
  ...
  #pragma omp parallel for num_threads(n_omp_threads) private(start_x,start_y,end_x,end_y,thread_num)

  for(thread_num = 0; thread_num < n_omp_threads; thread_num++){

     start_x = cells_per_domain_x * (thread_num / nDomains_y) ;
     start_y = cells_per_domain_y * (thread_num % nDomains_y) ;
...
void game(int w, int h) {
  ...
  for (t=0;t<TimeSteps;t++) {
    ...
    evolve_mpi_parallel(currentfield,newfield,w,h);
