...
void Filling_produceGun(Field* field, int value, int xpos, int ypos) {
  ...
  Field_setCell(field, 0, 0, value); //debugging
...
void Filling_binaryRandomizeFrame(Field* field, int value) {
  ...
  printf("inner size in dim Y = %i\n",Field_getInnerSizeInDim(field, Y));
  printf("inner size in dim X = %i\n",Field_getInnerSizeInDim(field, X));

  // for (y = offset; y < Field_getInnerSizeInDim(field, Y) - offset; y++) {
  //   for (x = offset; x < Field_getInnerSizeInDim(field, X) - offset; x++) {
      for (y = 0; y < Field_getSizeInDim(field, Y); y++) {
        for (x = 0; x < Field_getSizeInDim(field, X); x++) {
